...
const UPDATE_TEMP_DIR: &str = "/tmp/.bgdeskupdate";
...
// remove just one app from all the permissions: tccutil reset All br.com.boagestao.bgdesk
...
fn correct_app_name(s: &str) -> String {
    ...
    if let Some(bundleid) = get_bundle_id() {
        s = s.replace("br.com.boagestao.bgdesk", &bundleid);
    }
    s = s.replace("bgdesk", &crate::get_app_name().to_lowercase());
...
pub fn start_os_service() {
    ...
    /* // mouse/keyboard works in prelogin now with launchctl asuser.
       ...
       // one way is to start with `launchctl asuser <uid> open -n -a /Applications/BGDesk.app/ --args --cm`,
       // this way --cm is started with the user privilege, but we will have problem to start another BGDesk.app
...
pub fn update_me() -> ResultType<()> {
    ...
    let cmd = std::env::current_exe()?;
    // BGDesk.app/Contents/MacOS/BGDesk
    let app_dir = cmd
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
    ...
    if is_installed_daemon && !is_service_stopped {
        ...
        let update_body = format!(
            r#"
do shell script "
pgrep -x 'BGDesk' | grep -v {} | xargs kill -9 && rm -rf /Applications/BGDesk.app && ditto '{}' /Applications/BGDesk.app && chown -R {}:staff /Applications/BGDesk.app && xattr -r -d com.apple.quarantine /Applications/BGDesk.app
" with prompt "BGDesk wants to update itself" with administrator privileges
    "#,
            std::process::id(),
            app_dir,
            get_active_username()
...
fn extract_dmg(dmg_path: &str, target_dir: &str) -> ResultType<()> {
    let mount_point = "/Volumes/BGDeskUpdate";
...
fn update_extracted(target_dir: &str) -> ResultType<()> {
    let exe_path = format!("{}/BGDesk.app/Contents/MacOS/BGDesk", target_dir);
    let _child = unsafe {
